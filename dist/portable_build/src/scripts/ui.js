...
class UIManager {
    constructor() {
        ...
        // Ensure defensive helper functions exist to avoid unsafe char code usage
        if (typeof window !== 'undefined') {
            if (!window.safeLetter) {
                window.safeLetter = function(n) {
                    if (typeof n !== 'number' || !isFinite(n) || n < 0) return '';
                    return String.fromCharCode(65 + n);
                };
            }

            if (!window.normalizeString) {
                window.normalizeString = function(s) {
                    return (s || '').toString().trim().toLowerCase().replace(/\s+/g, ' ');
                };
            }

            if (!window.resolveCorrectDisplay) {
                window.resolveCorrectDisplay = function(question) {
                    const opts = Array.isArray(question.options) ? question.options : Object.values(question.options || {});
                    let idx = -1;

                    // Prefer numeric correct index
                    if (typeof question.correct === 'number') {
                        idx = question.correct;
                    } else if (typeof question.correct === 'string') {
                        const c = question.correct.trim();
                        if (/^[A-Z]$/i.test(c)) {
                            idx = c.toUpperCase().charCodeAt(0) - 65;
                        } else {
                            const norm = window.normalizeString(c);
                            for (let i = 0; i < opts.length; i++) {
                                if (window.normalizeString(opts[i]) === norm) { idx = i; break; }
                            }
                        }
                    }

                    // Try matching question.answer if still unresolved
                    if (idx === -1 && question.answer) {
                        const normA = window.normalizeString(question.answer);
                        for (let i = 0; i < opts.length; i++) {
                            if (window.normalizeString(opts[i]) === normA) { idx = i; break; }
                        }
                    }

                    // Final fallbacks
                    const letter = (typeof window !== 'undefined' && window.safeLetter) ? window.safeLetter(idx) : (idx >= 0 ? String.fromCharCode(65 + idx) : '');
                    const text = (idx >= 0 && opts[idx]) ? opts[idx] : (question.answer || '');
                    return { index: idx, letter: letter, text: text };
                };
            }
        }
    }
