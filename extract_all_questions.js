...
// Function to extract questions from a specific array
function extractQuestions(arrayName) {
        ...
        if (char === '{') {
            ...
            if (braceCount === 0) {
                ...
                try {
                    ...
                    // If JSON parse fails, try to eval (dangerous but necessary for JS objects)
                    try {
                        question = eval('(' + questionStr + ')');
                        questions.push(question);
                    } catch (e2) {
                        console.log(`Failed to parse question in ${arrayName}:`, questionStr.substring(0, 100));
                    }
                }
            }
        }
...
// Extract from all arrays, including unnamed ones at the end
const allArrays = [];
const arrayRegex = /\[([\s\S]*?)\]/g;
let match;
while ((match = arrayRegex.exec(content)) !== null) {
    if (match[1].includes('id:') || match[1].includes('"id"')) {
        allArrays.push(match[1]);
    }
}

console.log(`Found ${allArrays.length} question arrays`);

const allQuestions = [];
allArrays.forEach((arrayContent, index) => {
    const questions = [];
    let braceCount = 0;
    let start = 0;
    let inString = false;
            ...
            if (braceCount === 0) {
                ...
                try {
                    ...
                    try {
                        ...
                        console.log(`Failed to parse question in array ${index}:`, questionStr.substring(0, 100));
                    }
                }
            }
        }
    ...
    console.log(`Array ${index}: ${questions.length} questions`);
...
const convertedQuestions = allQuestions.map(q => {
    ...
    const test = q.test || q.Test || 1;
    const id = q.id;
    const question = q.question;
    const options = q.options;
    let correct = q.correct || q.answer || q.correct_answer;

    // Convert correct answer to index
    if (typeof correct === 'string') {
        correct = options.indexOf(correct);
        if (correct === -1) {
            console.log('Could not find correct answer for question:', id, correct);
            correct = 0; // default
        }
    }
...
// Output for STANDALONE.html
let output = 'window.testData = {
...
// Write to file
fs.writeFileSync('converted_questions.js', output);
console.log('Converted questions written to converted_questions.js');