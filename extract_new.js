...
// Prepare modified content for named arrays
let modifiedContent = content.replace(/export const (\w+) = /g, 'global.$1 = ');

// Remove the JSON arrays at the end (everything after PerformanceQuestions)
const perfIndex = modifiedContent.indexOf('global.PerformanceQuestions = [');
if (perfIndex !== -1) {
  const perfEnd = modifiedContent.indexOf('];', perfIndex) + 2;
  modifiedContent = modifiedContent.substring(0, perfEnd);
}

// Write to debug
fs.writeFileSync('debug.js', modifiedContent);

// Evaluate to define the variables globally
...
// Now get the arrays
let arrays = [global.AGKQuestions, global.AirLawQuestions, global.AONQuestions, global.FLTPLNQuestions, global.HPLQuestions, global.InstQuestions, global.MassBalQuestions, global.MetQuestions, global.OpsQuestions, global.PoFQuestions, global.RadNavQuestions, global.GenNavQuestions, global.PerformanceQuestions];

console.log(`Found ${arrays.length} named arrays`);

let allQuestions = [];
const categoryNames = ['AGKQuestions', 'AirLawQuestions', 'AONQuestions', 'FLTPLNQuestions', 'HPLQuestions', 'InstQuestions', 'MassBalQuestions', 'MetQuestions', 'OpsQuestions', 'PoFQuestions', 'RadNavQuestions', 'GenNavQuestions', 'PerformanceQuestions'];
const categoryMap = {
  AGKQuestions: 'aircraft-general-test-5',
  AirLawQuestions: 'air-law-test-5',
  AONQuestions: 'aircraft-general-test-5', // assuming
  FLTPLNQuestions: 'flight-planning-test-5',
  HPLQuestions: 'human-performance-test-5',
  InstQuestions: 'instrumentation-test-5',
  MassBalQuestions: 'mass-balance-test-5',
  MetQuestions: 'meteorology-test-5',
  OpsQuestions: 'aircraft-general-test-5', // assuming
  PoFQuestions: 'principles-of-flight-test-5',
  RadNavQuestions: 'radio-navigation-test-5',
  GenNavQuestions: 'general-navigation-test-5',
  PerformanceQuestions: 'performance-test-5'
};

arrays.forEach((array, index) => {
  const name = categoryNames[index];
  const categoryKey = categoryMap[name];
  console.log(`Processing ${name}: ${array.length} questions`);
  array.forEach(q => {
    allQuestions.push({
      ...q,
      category: categoryKey,
      correct: q.correct_answer ? q.options.indexOf(q.correct_answer) : (q.correct !== undefined ? q.correct : (q.answer ? q.options.indexOf(q.answer) : 0)),
      explanation: q.explanation || ''
    });
  });
});

// Now, handle the additional JSON arrays
const jsonRegex = /\[\s*\{[\s\S]*?\}\s*\]/g;
let match;
let jsonCount = 0;
while ((match = jsonRegex.exec(content)) !== null) {
  jsonCount++;
  console.log(`Match ${jsonCount}: length ${match[0].length}`);
  try {
    const jsonArray = eval(match[0]);
    if (Array.isArray(jsonArray) && jsonArray.length > 0 && typeof jsonArray[0] === 'object' && jsonArray[0].question_number) {
      console.log(`Processing JSON array: ${jsonArray.length} questions`);
      jsonArray.forEach(q => {
        const categoryKey = q.category.toLowerCase().replace(/\s+/g, '-').replace('and', '').replace('limitations', '') + '-test-5'; // e.g., flight-planning-and-monitoring -> flight-planning-test-5
        allQuestions.push({
          ...q,
          category: categoryKey,
          correct: q.correct_answer ? q.options.indexOf(q.correct_answer) : 0,
          explanation: q.explanation || ''
        });
      });
    }
  } catch (e) {
    console.log(`Failed to parse match ${jsonCount}: ${e.message}`);
  }
}
...
console.log('File exists:', fs.existsSync('STANDALONE.html'));
...
console.log('File length:', htmlContent.length);
console.log('Contains testData:', htmlContent.includes('testData'));
console.log('Contains window.testData:', htmlContent.includes('window.testData'));

// Parse the existing testData (simple way: find the object)
...
console.log('Extracted testData length:', testDataStr.length);

let existingTestData;
try {
  existingTestData = eval('(' + testDataStr + ')');
} catch (e) {
...
Object.keys(categories).forEach(cat => {
  if (existingTestData[cat]) {
    // Find the test with questions
    const test = existingTestData[cat].tests.find(t => t.questions);
    if (test) {
      test.questions.push(...categories[cat]);
      console.log(`Added ${categories[cat].length} questions to ${cat}`);
    }
  } else {
    // Add new category
    existingTestData[cat] = {
    ...
    console.log(`Added new category ${cat} with ${categories[cat].length} questions`);
  }
});

// Replace in HTML
const fullTestData = htmlContent.substring(startIndex, objEnd + 2); // window.testData = {
      ...
      },
      {
        ...
        "questions": [
          ...
          },
          {
            ...
            "answer": "50 m between 300 m and 800 m",
            "correct": 2,
            "explanation": ""
          },
          {
...
const newTestDataStr = JSON.stringify(existingTestData, null, 2);
