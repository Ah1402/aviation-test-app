...
with open('441.js', 'r', encoding='utf-8') as f:
    content = f.read()

# Function to find array boundaries
def find_array_bounds(content, start_pos):
    brace_count = 0
    in_string = False
    escape_next = False
    start_found = False
...
for match in matches:
    ...
    start_pos = match.end() - 1  # Position after '='

    # Find the array bounds
    array_start, array_end = find_array_bounds(content, start_pos)
    if array_start is not None and array_end is not None:
        array_content = content[array_start:array_end]

        try:
            # Convert JS to JSON
            json_content = re.sub(r"'", '"', array_content)  # Replace single quotes
            json_content = re.sub(r'(\w+):', r'"\1":', json_content)  # Quote property names

            # Parse the JSON array
            questions = json.loads(json_content)
            all_questions[array_name] = questions
            print(f"Loaded {array_name}: {len(questions)} questions")
            total_count += len(questions)
        except json.JSONDecodeError as e:
            print(f"Error parsing {array_name}: {e}")
            print(f"Content sample: {array_content[:200]}...")

print(f"\nTotal questions found: {total_count}")
print(f"Total categories: {len(all_questions)}")

# Now convert to app format
def convert_questions(questions, category_key, category_name, icon):
    tests = {}

    for q in questions:
        test_key = f"{category_key}-test-{q['test']}"
        ...
        try:
            ...
            correct_index = 0  # Default to first option if not found

        tests[test_key]["questions"].append({
            "id": q["id"],
            "question": q["question"],
            "options": q["options"],
...
categories = [
    ("AGKQuestions", 'aircraft-general-knowledge', 'Aircraft General Knowledge', 'fas fa-plane'),
    ("AirLawQuestions", 'air-law', 'Air Law', 'fas fa-gavel'),
    ("AONQuestions", 'aon-aviation', 'AON Aviation Knowledge', 'fas fa-graduation-cap'),
    ("FLTPLNQuestions", 'flight-planning', 'Flight Planning & Monitoring', 'fas fa-route'),
    ("HPLQuestions", 'human-performance', 'Human Performance & Limitations', 'fas fa-user-md'),
    ("InstQuestions", 'instrument-rating', 'Instrument Rating', 'fas fa-compass'),
    ("MassBalQuestions", 'mass-and-balance', 'Mass and Balance', 'fas fa-balance-scale'),
    ("MetQuestions", 'meteorology', 'Meteorology', 'fas fa-cloud'),
    ("OpsQuestions", 'operational-procedures', 'Operational Procedures', 'fas fa-cogs'),
    ("PoFQuestions", 'principles-of-flight', 'Principles of Flight', 'fas fa-wind'),
    ("RadNavQuestions", 'radio-navigation', 'Radio Navigation', 'fas fa-satellite'),
    ("GenNavQuestions", 'general-navigation', 'General Navigation', 'fas fa-map'),
    ("PerformanceQuestions", 'performance', 'Performance', 'fas fa-chart-line')
]

test_data = {}
processed_questions = 0

for array_name, key, name, icon in categories:
    if array_name in all_questions:
        data = convert_questions(all_questions[array_name], key, name, icon)
        test_data.update(data)
        processed_questions += len(all_questions[array_name])

# Write to file
with open('testData_complete.js', 'w', encoding='utf-8') as f:
    f.write(f'window.testData = {
...
print(f"Total categories: {len([c for c in categories if c[0] in all_questions])}")