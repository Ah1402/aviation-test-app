...
$backupFile = "src\data\testData_backup.js"
...
if ($startLine -ge 0 -and $endLine -ge 0) {
    ...
    Write-Host "  Old section: lines $($startLine + 1) to $($endLine + 1) ($oldCount lines)"
    Write-Host "  New section: $($replacementLines.Count) lines"
} else {
    Write-Host "ERROR: Could not find performance section boundaries"
    Write-Host "  Start line: $startLine"
    Write-Host "  End line: $endLine"
}
