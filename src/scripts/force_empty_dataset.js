(function(){
  ...
  "performance": {
    ...
    // Clear category cards if present
    const container = document.querySelector('.test-categories');
    if (container) {
      container.innerHTML = '<div class="no-data" style="padding:16px;color:#64748b;">No questions available.</div>';
    }

    // Zero out common counters
    ['stat-questions-count','about-questions-count','total-questions','studyTotalQuestions']
      .forEach(id => { const el = document.getElementById(id); if (el) el.textContent = '0'; });

    // Rebuild search index if app exposes it
    try { if (window.app && typeof window.app.rebuildSearchEngine === 'function') window.app.rebuildSearchEngine(); } catch {}

    console.log('[purge] Questions and answers removed at runtime.');
  } catch (e) {
    console.warn('[purge] Failed to force empty dataset', e);
  }
})();
